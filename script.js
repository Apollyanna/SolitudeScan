/* =========================================================================
   SOLITUDESCAN — SCRIPT PRINCIPAL v3.0 (CONSOLIDADO E LIMPO)
   Todas as funcionalidades originais + todas as correções profissionais.
...
// Substitui via.placeholder.com (serviço morto em 2024)
...
const CAPA_PLACEHOLDER = "data:image/svg+xml," + encodeURIComponent(
...
const PAGINA_PLACEHOLDER = "data:image/svg+xml," + encodeURIComponent(
...
let usuario = {
  ...
  planoVip: null,
  vipExpiraEm: null,
  favoritos: (() => { try { return JSON.parse(localStorage.getItem('solitude_favoritos') || '[]'); } catch (e) { return []; } })(),
  historico: (() => { try { return JSON.parse(localStorage.getItem('solitude_historico') || '[]'); } catch (e) { return []; } })()
};
...
let ordemCapitulosInvertida = false;
...
function todasAsObras() {
  const mapa = new Map();
  try {
    if (typeof listaObras !== 'undefined' && Array.isArray(listaObras)) {
      listaObras.forEach(o => mapa.set(String(o.id), o));
    }
    if (typeof obrasRemotas !== 'undefined' && Array.isArray(obrasRemotas)) {
      obrasRemotas.forEach(o => mapa.set(String(o.id), o));
    }
  } catch (e) {}
  return [...mapa.values()];
}
...
/* =========================================================================
   4. SEGURANÇA — CLICKJACKING + BLOQUEADOR DE ANÚNCIOS + PROTEÇÕES LEITOR
   ========================================================================= */
...
function iniciarBloqueadorAnuncios() {
    ...
    mutations.forEach(mutation => {
      mutation.addedNodes.forEach(node => {
        ...
        if (node.tagName === 'IFRAME') {
          ...
          if (palavrasBloqueio.some(p => src.includes(p))) {
            node.remove();
            return;
          }
        }
        ...
        if (node.tagName === 'DIV' || node.tagName === 'INS' || node.tagName === 'ASIDE') {
          ...
          if (palavrasBloqueio.some(p => id.includes(p) || className.includes(p))) {
            node.remove();
            return;
          }
        }
  ...
  setInterval(() => {
    ...
    document.querySelectorAll(seletores).forEach(el => {
      ...
      if (palavrasBloqueio.some(p => id.includes(p) || className.includes(p) || src.includes(p))) {
        el.remove();
      }
    });
  }, 1500);
...
async function carregarPerfilRemoto(userId) {
  ...
  try {
    ...
    if (data) {
      ...
      if (data.full_name) usuario.nome = data.full_name;
      if (data.phone) usuario.telefone = data.phone;
      if (data.avatar_url) usuario.foto = data.avatar_url;
    }
  } catch (e) {
...
function atualizarPerfilTela() {
  ...
  if (usuario.logado) {
    ...
    if (avatarImg) avatarImg.src = usuario.foto || AVATAR_SVG;
...
async function abrirConfigPerfil() {
  ...
  if (prev) prev.src = usuario.foto || AVATAR_SVG;
...
/* =========================================================================
   8. TEMA (CLARO/ESCURO + AUTOMÁTICO POR HORÁRIO)
   ========================================================================= */
...
function aplicarTemaAutomatico() {
  try {
    if (localStorage.getItem('solitude_theme')) return; // respeita escolha manual
...
/* =========================================================================
   10. SINCRONIZAÇÃO DE SESSÃO ENTRE ABAS (onAuthStateChange)
   ========================================================================= */

function ativarSincronizacaoSessao() {
  if (!_supabase) return;
  try {
    _supabase.auth.onAuthStateChange((evento, sessao) => {
...
/* =========================================================================
   11. CATÁLOGO — MAPEAMENTO SUPABASE → LOCAL
   ========================================================================= */

let obrasRemotas = [];
// listaObras nasce VAZIA — tudo vem do Supabase (sem obras demo hardcoded)
let listaObras = [];
const CATALOGO_PADRAO = [];

function aplicarOverridesLocais() {
  // Mantido por compatibilidade; listaObras já nasce vazia
  try {
...
function mudarCamposSupabaseParaLocal(obra) {
  ...
  return {
    ...
    capa: obra.cover_url || CAPA_PLACEHOLDER,
...
function capaComFallback(obra) {
  return (obra && obra.capa) ? obra.capa : CAPA_PLACEHOLDER;
}

/* =========================================================================
   12. CARREGAMENTO E RENDERIZAÇÃO DO CATÁLOGO
   ========================================================================= */
...
async function carregarObras() {
  ...
  container.innerHTML = '<div style="grid-column: 1/-1; text-align:center; padding:40px; color:var(--texto-claro);">' +
    '<i class="fa-solid fa-spinner fa-spin fa-2x" style="color:var(--primaria); margin-bottom:10px;"></i>' +
    '<p>Carregando obras...</p></div>';
  ...
  if (_supabase) {
    try {
      ...
      console.warn('[SolitudeScan] Erro ao buscar obras:', err);
    }
  }

  // Fallback local apenas se Supabase falhar completamente
  if (obras.length === 0 && !termoBusca && listaObras.length > 0) {
    obras = listaObras.slice();
    if (generoAtual === 'Favoritos') obras = obras.filter(o => usuario.favoritos.includes(o.id));
    else if (generoAtual !== 'Todos') obras = obras.filter(o => o.genero === generoAtual);
    if (statusFiltro !== 'Todos') obras = obras.filter(o => o.status === statusFiltro);
    obras = ordenarObras(obras);
  }

  renderizarGridObras(obras);
  limparPlaceholdersMortos();
}
...
function renderizarGridObras(obras) {
  ...
  if (!obras || obras.length === 0) {
    container.innerHTML = '<div style="grid-column: 1/-1; text-align:center; padding:30px; color:var(--texto-claro);">' +
      '<i class="fa-solid fa-book-open" style="font-size:2rem; color:var(--primaria); margin-bottom:10px;"></i>' +
      '<p>Nenhuma obra encontrada.</p></div>';
    return;
  }

  container.innerHTML = obras.map(obra => {
    const fav = usuario.favoritos.includes(obra.id);
    return '<div class="manga-card" onclick="abrirDetalhesObra(\'' + obra.id + '\')" role="button" tabindex="0" aria-label="' + escaparHtml(obra.titulo) + '">' +
      (obra.isVip ? '<div class="vip-badge">VIP</div>' : '') +
      (fav ? '<div class="fav-badge"><i class="fa-solid fa-heart" style="color:var(--primaria);"></i></div>' : '') +
      '<img src="' + escaparHtml(capaComFallback(obra)) + '" alt="Capa de ' + escaparHtml(obra.titulo) + '" class="manga-img" loading="lazy">' +
...
function filtrarGenero(genero, el) {
  generoAtual = genero;
  document.querySelectorAll('#genreChipsContainer .chip').forEach(c => c.classList.remove('active'));
  if (el) el.classList.add('active');
  carregarObras();
}
...
function definirOrdem(valor, rotulo) {
  ordemFiltro = valor;
  const label = document.getElementById('labelOrdem');
...
/* =========================================================================
   13. TENDÊNCIAS, RANKING, CONCLUÍDAS, CONTINUE LENDO
   ========================================================================= */
...
function criarItemCarousel(obra, comUltimoCap = false) {
  ...
  return '<div class="carousel-item">' +
    ...
    (obra.isVip ? '<div class="vip-badge">VIP</div>' : '') +
    '<img src="' + escaparHtml(capaComFallback(obra)) + '" alt="' + escaparHtml(obra.titulo) + '" class="manga-img" loading="lazy" style="height:180px;">' +
    '<div class="manga-info">' +
    '<p class="manga-title">' + escaparHtml(obra.titulo) + '</p>' +
...
async function renderizarTendencias() {
  ...
  container.innerHTML = tendencia.length > 0
    ? tendencia.map(obra => criarItemCarousel(obra)).join('')
    : '<p style="color:var(--texto-claro); font-size:0.8rem;">Nenhuma tendência ainda.</p>';
...
async function renderizarRanking() {
  ...
  container.innerHTML = top10.map((obra, idx) =>
    ...
    (obra.isVip ? '<div class="vip-badge">VIP</div>' : '') +
    '<img src="' + escaparHtml(capaComFallback(obra)) + '" alt="' + escaparHtml(obra.titulo) + '" class="manga-img" loading="lazy">' +
    '<div class="manga-info">' +
    '<p class="manga-title">' + escaparHtml(obra.titulo) + '</p>' +
...
function renderizarConcluidas() {
  ...
  if (concluidas.length === 0) {
    container.innerHTML = '<p style="grid-column:1/-1; font-size:0.8rem; color:var(--texto-claro); text-align:center; padding:20px;">Nenhuma obra concluída ainda.</p>';
    return;
  }
...
/* =========================================================================
   14. HERO BANNER — CARROSSEL DINÂMICO + SWIPE + AUTO-ROTATE
   ========================================================================= */
...
async function carregarHeroBanner() {
  ...
  if (!heroInterval && heroObras.length > 1) {
    heroInterval = setInterval(() => {
      if (heroObras.length > 1) {
        rotacionarHero();
        atualizarDotsHero();
      }
    }, 5000);
...
function rotacionarHero() {
  ...
  section.style.background = 'linear-gradient(180deg, rgba(10,5,11,0.2) 0%, var(--fundo) 100%), url("' + escaparHtml(obra.capa) + '") center/cover no-repeat';
...
function atualizarDotsHero() {
  ...
  if (!dots) {
    ...
    dots.id = 'heroDots';
    dots.style.cssText = 'position:absolute; bottom:12px; left:0; right:0; display:flex; justify-content:center; gap:6px; z-index:5;';
    section.style.position = 'relative';
    section.appendChild(dots);
  }
  const idxAtual = (heroIndice - 1) % heroObras.length;
  dots.innerHTML = heroObras.map((_, i) =>
    '<span onclick="event.stopPropagation(); heroIndice=' + i + '; rotacionarHero(); atualizarDotsHero();" style="width:8px; height:8px; border-radius:50%; cursor:pointer; background:' + (i === idxAtual ? 'var(--primaria)' : 'rgba(255,255,255,0.35)') + '; transition:all 0.3s;"></span>'
  ).join('');
}

function ativarSwipeHero() {
  if (window.__swipeHeroAtivo) return;
  window.__swipeHeroAtivo = true;
...
/* =========================================================================
   15. DETALHES DA OBRA — PÁGINA COMPLETA
   ========================================================================= */

async function abrirDetalhesObra(id) {
  let obra = todasAsObras().find(o => String(o.id) === String(id));

  // Busca no Supabase se não estiver no cache
  if (!obra && _supabase && ehUuid(id)) {
    try {
      const { data } = await _supabase.from('works').select('*, work_genres(genres(name))').eq('id', id).single();
      if (data) {
        obra = mudarCamposSupabaseParaLocal(data);
        obrasRemotas.push(obra);
      }
    } catch (e) {}
  }
  ...
  if (_supabase) {
    try {
      const { data: caps } = await _supabase.from('chapters')
        .select('*')
        .eq('work_id', id)
        .eq('is_published', true)
  ...
  // Popula dados na modal
  const set = (idEl, v) => { const el = document.getElementById(idEl); if (el) el.textContent = v; };
  set('detalhesTitulo', obra.titulo);
  const capa = document.getElementById('detalhesCapa');
  if (capa) capa.src = capaComFallback(obra);
  set('detalhesSinopse', obra.sinopse || 'Sem sinopse disponível.');
  ...
  try {
    const slug = obra.slug || String(obra.titulo).toLowerCase()
      .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
      .replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
    history.pushState({ obraId: id }, '', '?obra=' + (slug || id));
  } catch (e) {}
}
...
async function renderizarCapitulosInfo(obra) {
  ...
  if (totalCaps === 0) totalCaps = capitulosObraAtual.length;

  const dataUltimo = ultimoCap && ultimoCap.release_at
    ? dataRelativa(ultimoCap.release_at)
    : '--';
...
function renderizarBadgesDetalhes(obra) {
  ...
  sec.innerHTML = '<span style="background:' + corStatus + '; color:#fff; padding:3px 10px; border-radius:10px; font-size:0.65rem; font-weight:800; text-transform:uppercase;">' + escaparHtml(obra.status) + '</span>' +
...
function atualizarBolinhasAvaliacao(nota) {
  ...
  dots.forEach((el, idx) => {
    ...
    el.style.background = idx < Math.round(nota) ? 'var(--primaria)' : 'transparent';
    el.style.border = '2px solid var(--primaria)';
  });
}
...
function atualizarListaCapitulos() {
  ...
  const lidos = new Set((usuario.historico || []).map(h => String(h.chapter_id)));

  container.innerHTML = caps.map((cap, idx) => {
    const realIndex = ordemInvertida ? (capitulosObraAtual.length - 1 - idx) : idx;
    const lido = lidos.has(String(cap.id));
    return '<button class="chip" style="width:100%; text-align:left; border-radius:6px; display:flex; justify-content:space-between; align-items:center; color:' + (lido ? 'var(--texto-claro)' : 'var(--texto)') + ';" onclick="abrirLeitor(' + realIndex + ')">' +
      '<span>' + escaparHtml(cap.numero) + (cap.is_vip ? ' 👑' : '') + '</span>' +
      '<div style="display:flex; gap:8px; align-items:center;">' +
      (lido ? '<span style="font-size:0.6rem; color:var(--sucesso);"><i class="fa-solid fa-check"></i> Lido</span>' : '') +
...
async function alternarFavorito() {
  if (!usuario.logado) {
    mostrarToast('Entre na conta para favoritar.', 'alerta');
    toggleModal('loginModal');
    return;
  }
...
async function carregarFavoritosUsuario() {
  if (!_supabase || !usuario.logado) {
    try { usuario.favoritos = JSON.parse(localStorage.getItem('solitude_favoritos') || '[]'); } catch (e) {}
    return;
  }
  try {
    const { data } = await _supabase.from('favorites').select('work_id').eq('user_id', usuario.id);
    if (data) usuario.favoritos = data.map(f => String(f.work_id));
  } catch (e) {}
}

/* =========================================================================
   17. HISTÓRICO DE LEITURA
   ========================================================================= */
...
function adicionarHistorico(obra) {
  ...
  usuario.historico.unshift({
    ...
    titulo: obra.titulo,
    capa: obra.capa,
    chapter_id: cap ? cap.id : null,
    ultimoCap: cap ? cap.numero : 'Capítulo 1',
  ...
  if (_supabase && usuario.logado && cap) {
    salvarProgressoLeituraSupabase(obra.id, cap.id, 1);
  }
...
async function carregarHistoricoUsuario() {
  if (!_supabase || !usuario.logado) {
    try { usuario.historico = JSON.parse(localStorage.getItem('solitude_historico') || '[]'); } catch (e) {}
    return;
  }
  try {
    const { data } = await _supabase
      .from('reading_history')
      .select('*, works(id, title, cover_url, slug)')
      .eq('user_id', usuario.id)
      .order('updated_at', { ascending: false })
      .limit(50);
    if (data) {
      usuario.historico = data.map(h => ({
        id: h.work_id,
        titulo: h.works ? h.works.title : 'Obra',
        capa: h.works ? h.works.cover_url : '',
        chapter_id: h.chapter_id,
        ultimoCap: h.last_page ? ('Página ' + h.last_page) : '',
        data: h.updated_at
      }));
    }
  } catch (e) {}
...
function renderizarHistorico() {
  ...
  if (!usuario.historico || usuario.historico.length === 0) {
    c.innerHTML = '<p style="grid-column:1/-1; font-size:0.8rem; color:var(--texto-claro); text-align:center;">Seu histórico está vazio.</p>';
    return;
  }
  c.innerHTML = usuario.historico.slice(0, 12).map(h =>
    '<div class="manga-card" onclick="abrirDetalhesObra(\'' + h.id + '\')" role="button" tabindex="0">' +
    '<img src="' + escaparHtml(h.capa || AVATAR_SVG) + '" alt="' + escaparHtml(h.titulo || '') + '" loading="lazy">' +
    '<div class="manga-info">' +
    '<p class="manga-title">' + escaparHtml(h.titulo || 'Obra') + '</p>' +
    '<p class="manga-cap">' + dataRelativa(h.data) + '</p>' +
    '</div></div>'
  ).join('');
}

/* =========================================================================
   18. LEITOR — ABERTURA + VERIFICAÇÃO VIP
   ========================================================================= */

async function abrirLeitor(indice) {
  let obra = todasAsObras().find(o => String(o.id) === String(obraAtualId));
  ...
  // Proteção extra no nível do capítulo
  if (cap.is_vip && !usuario.isVip && !usuario.isAdmin) {
    mostrarToast('Este capítulo é exclusivo VIP.', 'alerta');
    abrirModalVip();
    return;
  }
  ...
  // Carrega páginas (fallback do Supabase se faltar)
  ...
  if (!cap.paginas || cap.paginas.length === 0) {
    cascataContainer.innerHTML = '<div style="text-align:center; padding:40px; color:var(--texto-claro);">' +
      '<i class="fa-solid fa-image" style="font-size:2rem; color:var(--primaria); margin-bottom:10px;"></i>' +
      '<p>Este capítulo ainda não tem páginas.</p></div>';
    return;
  }

  // Renderiza: 2 primeiras com src imediato, resto com data-src (lazy)
  cascataContainer.innerHTML = cap.paginas.map((pag, i) => {
    const attr = i < 2 ? 'src="' + escaparHtml(pag) + '"' : 'data-src="' + escaparHtml(pag) + '"';
    return '<img ' + attr + ' loading="lazy" class="pagina-capitulo" alt="Página ' + (i + 1) + '" style="width:' + nivelZoomLeitor + '%; max-width:100%; display:block; margin:0 auto 10px auto; border-radius:4px; background:#111; min-height:120px;">';
  }).join('');

  ativarLazyLoadingLeitor(cascataContainer, cap.paginas);
  montarRodapeLeitor();
  restaurarProgressoPixel();

  if (obra) adicionarHistorico(obra);
...
/* =========================================================================
   19. LAZY LOADING + PRE-FETCH + RETRY + LIBERAÇÃO DE MEMÓRIA
   ========================================================================= */

function ativarLazyLoadingLeitor(cascata, paginas) {
  const imgs = Array.prototype.slice.call(cascata.querySelectorAll('img.pagina-capitulo'));
    ...
    if (!img.dataset.retry) {
      ...
      img.addEventListener('error', function () {
        ...
        if (n < 3 && base) {
          ...
          setTimeout(() => {
            img.src = base + (base.indexOf('?') === -1 ? '?' : '&') + 'r=' + (n + 1);
          }, 700 * (n + 1));
        }
      });
    }
  ...
  // IntersectionObserver: carrega quando se aproxima + pre-fetch das próximas 3
  if (cascata.__obs) cascata.__obs.disconnect();
  cascata.__obs = new IntersectionObserver(entries => {
    entries.forEach(en => {
      ...
      for (let i = 1; i <= 3; i++) {
        const nx = imgs[idx + i];
        if (nx && !nx.src && nx.dataset.src) {
          const pre = new Image();
          pre.src = nx.dataset.src;
        }
      }
      cascata.__obs.unobserve(img);
    });
  }, { root: cascata, rootMargin: '400px' });

  imgs.forEach(img => {
    if (!img.src) cascata.__obs.observe(img);
  });

  // Libera memória: remove src de imagens 2+ telas acima do scroll
  if (!cascata.__memAtivo) {
    cascata.__memAtivo = true;
    cascata.addEventListener('scroll', debounce(() => {
      const limite = cascata.scrollTop - cascata.clientHeight * 2;
      imgs.forEach(img => {
        if (img.src && (img.offsetTop + img.offsetHeight) < limite) {
          img.dataset.src = img.src;
          img.removeAttribute('src');
          if (cascata.__obs) cascata.__obs.observe(img);
        }
      });
    }, 400), { passive: true });
  }

  // Progresso visual + salvamento
  if (!cascata.__progAtivo) {
    cascata.__progAtivo = true;
    cascata.addEventListener('scroll', debounce(() => {
      const total = cascata.scrollHeight - cascata.clientHeight;
      const pct = total > 0 ? Math.round((cascata.scrollTop / total) * 100) : 0;
      const pEl = document.getElementById('readerProgress');
      if (pEl) pEl.textContent = pct + '%';
      const tEl = document.getElementById('readerTitle');
      const cEl = document.getElementById('readerCap');
      if (tEl) {
        document.title = '[' + pct + '%] ' + tEl.textContent + (cEl ? ' - ' + cEl.textContent : '') + ' | SolitudeScan';
      }
...
function preCarregarImagensCapitulo(urlsPaginas) {
  ...
  urlsPaginas.slice(0, 3).forEach(url => {
    const img = new Image();
    img.src = url;
  });
}
...
function fecharLeitor() {
  ...
  toggleModal('readerModal');
  document.title = 'SolitudeScan — Ler Manhwas, Mangás e Manhuás Online Grátis';
}
...
function alternarTelaCheia() {
  if (!document.fullscreenElement) {
    document.documentElement.requestFullscreen().catch(err => {
      mostrarToast('Não foi possível ativar a tela cheia: ' + err.message, 'erro');
    });
  } else {
...
function alternarModoImersivo() {
  ...
  const reader = document.getElementById('readerModal');
  if (reader) reader.classList.toggle('auto-imersivo', modoImersivo);
  const top = document.querySelector('.reader-top-bar');
  const com = document.querySelector('.comments-section');
  if (top) top.style.display = modoImersivo ? 'none' : '';
  if (com) com.style.display = modoImersivo ? 'none' : '';
  mostrarToast(
...
function iniciarGestosLeitor() {
  ...
  if (!area || area.__gestosAtivos) return;
  area.__gestosAtivos = true;
...
function injetarEstiloAutoImersivo() {
  ...
  style.textContent = '#readerModal > * { transition: opacity 0.25s ease; } #readerModal.auto-imersivo > *:not([data-leitor-essencial]):not(.pagina-capitulo):not(.btn-fim-capitulo) { opacity: 0 !important; pointer-events: none !important; }';
  document.head.appendChild(style);
}

function iniciarAtalhosTecladoLeitor() {
  if (window.__atalhosAtivos) return;
  window.__atalhosAtivos = true;
  document.addEventListener('keydown', e => {
...
function iniciarGestosAvancadosLeitor() {
  const area = document.getElementById('readerCascataContainer');
  if (!area || area.__gestosAvancadosAtivos) return;
  area.__gestosAvancadosAtivos = true;
  ...
  // Auto-imersivo ao rolar para baixo
...
/* =========================================================================
   21. PROGRESSO DE LEITURA — SALVAR E RESTAURAR
   ========================================================================= */
...
function montarRodapeLeitor() {
  ...
  area.insertAdjacentHTML('beforeend',
    '<div class="btn-fim-capitulo" style="text-align:center; padding:24px 12px 48px;">' +
    (ehUltimo
...
async function compartilharObraAtual() {
  ...
  const slug = obra.slug || String(obra.titulo).toLowerCase()
    .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
    .replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
  const url = window.location.origin + '/?obra=' + (slug || obra.id);
...
function garantirBotaoCompartilhar() {
  ...
  if (!btn) {
    ...
    btn.style.cssText = (btnFav.style.cssText || '') + ' margin-top:8px; width:100%; background:var(--fundo-card); color:var(--texto); border:1px solid var(--borda); padding:8px; border-radius:6px; font-size:0.8rem; font-weight:700; cursor:pointer;';
...
function garantirBotaoCurtirObra() {
  ...
  if (!btn) {
    const btnLista = document.getElementById('btnAdicionarLista');
    if (!btnLista) return null;
    btn = document.createElement('button');
    btn.id = 'btnCurtirObra';
    btn.style.cssText = (btnLista.style.cssText || '') + ' margin-top:6px;';
    btnLista.parentNode.insertBefore(btn, btnLista.nextSibling);
  }
  return btn;
}
...
/* =========================================================================
   24. COMENTÁRIOS — ÁRVORE + RESPOSTAS + SPOILER + CURTIDAS + DENÚNCIAS
   ========================================================================= */

function garantirCheckboxSpoiler() {
  const form = document.querySelector('#readerModal form[onsubmit*="adicionarComentario"]') || document.querySelector('form[onsubmit*="adicionarComentario"]');
  if (!form || document.getElementById('chkSpoiler')) return;
  const wrap = document.createElement('label');
  wrap.style.cssText = 'display:flex; align-items:center; gap:5px; font-size:0.65rem; color:var(--texto-claro); margin-top:4px; cursor:pointer;';
  wrap.innerHTML = '<input type="checkbox" id="chkSpoiler" style="accent-color:var(--primaria); width:12px; height:12px;"> Marcar como spoiler';
...
async function carregarComentariosObra(obraId) {
  ...
  if (_supabase) {
    try {
      ...
      if (data && data.length > 0) {
        data.forEach(c => {
          comentariosCache[c.id] = Object.assign({}, c, { respostas: [] });
        });
...
function renderizarComentario(c, nivel) {
  ...
  const dataStr = c.created_at ? dataRelativa(c.created_at) : '';

  // Spoiler tag: texto borrado até clique
  let conteudo;
  if (c.spoiler) {
    conteudo = '<span onclick="this.style.filter=\'none\'; this.style.userSelect=\'text\'; this.title=\'\';" style="filter:blur(6px); cursor:pointer; user-select:none; display:inline-block;" title="Clique para revelar spoiler">⚠️ ' + escaparHtml(c.content) + '</span>';
  } else {
    conteudo = escaparHtml(c.content);
  }

  return '<div style="' + margem + '">' +
    ...
    '<strong style="color:var(--primaria);">' + (c.spoiler ? '⚠️ ' : '') + escaparHtml(c.user_name || 'Leitor') + '</strong>' +
    '<span style="font-size:0.6rem; color:var(--texto-claro);">' + dataStr + '</span>' +
    '</div>' +
    '<p style="margin:4px 0;">' + conteudo + '</p>' +
...
async function adicionarComentario(e) {
  ...
  // Rate limit: máximo 5 por minuto
  ...
  const chk = document.getElementById('chkSpoiler');
  const spoiler = !!(chk && chk.checked);
  ...
  if (_supabase) {
    try {
      ...
      if (usuario.id && ehUuid(usuario.id)) payload.user_id = usuario.id;
      const cap = capitulosObraAtual[indiceCapituloAtual];
      if (cap && cap.id && ehUuid(cap.id)) payload.chapter_id = cap.id;
      if (parentId && ehUuid(parentId)) payload.parent_id = parentId;
...
function garantirModalDenuncia() {
  ...
  modal.innerHTML = '<div class="modal-content" style="max-width:420px; width:92%; margin:auto; background:var(--fundo-card); border:1px solid var(--borda); border-radius:12px; padding:16px;">' +
...
/* =========================================================================
   28. URL AMIGÁVEL — POPSTATE (botão voltar)
   ========================================================================= */

function ativarPopstate() {
  if (window.__popstateAtivo) return;
  window.__popstateAtivo = true;
  window.addEventListener('popstate', e => {
    if (e.state && e.state.obraId) {
      abrirDetalhesObra(e.state.obraId);
    } else {
...
function processarLinksProfundos() {
  ...
  if (obraParam) {
    setTimeout(async () => {
      if (ehUuid(obraParam)) {
        abrirDetalhesObra(obraParam);
        return;
      }
      const obra = todasAsObras().find(o => (o.slug || '') === obraParam);
      if (obra) abrirDetalhesObra(obra.id);
    }, 900);
  }
}

/* =========================================================================
   29. PAINEL ADMINISTRATIVO — ABERTURA E PERMISSÃO
   ========================================================================= */
...
function injetarCampoPaginasAdmin() {
  ...
  div.innerHTML = '<label>Páginas do capítulo (pode selecionar várias)</label>' +
    '<div class="upload-dropzone" onclick="document.getElementById(\'admPaginasArquivo\').click()" role="button" tabindex="0" style="border:2px dashed var(--primaria); border-radius:10px; padding:16px; text-align:center; cursor:pointer;">' +
    '<i class="fa-solid fa-cloud-arrow-up" style="font-size:1.5rem; color:var(--primaria); margin-bottom:6px;"></i>' +
    '<p style="font-size:0.75rem; color:var(--texto-claro);">Clique ou arraste as imagens das páginas aqui</p>' +
    '<p id="admPaginasContagem" style="font-size:0.65rem; color:var(--primaria); margin-top:6px;"></p>' +
    '</div>' +
    '<input type="file" id="admPaginasArquivo" accept="image/*" multiple style="display:none;" onchange="document.getElementById(\'admPaginasContagem\').textContent=this.files.length+\' imagem(ns) selecionada(s)\'">';
...
/* =========================================================================
   30. DASHBOARD ADMIN — ESTATÍSTICAS
   ========================================================================= */
...
/* =========================================================================
   31. SOLICITAÇÕES DE PAGAMENTO PIX — APROVAR / RECUSAR
   ========================================================================= */
...
async function carregarSolicitacoesPagamento() {
  ...
  try {
    ...
    container.innerHTML = data.map(p =>
      ...
      '<div>' +
      '<strong style="font-size:0.75rem;">' + escaparHtml(p.nome || p.user_name || '') + '</strong>' +
      '<p style="font-size:0.65rem; color:var(--texto-claro); margin:2px 0 0;">' + escaparHtml(p.email) + '</p>' +
      '<p style="font-size:0.65rem; margin:2px 0 0;">' + escaparHtml(p.plano || p.plan_name || '') + ' — ' + escaparHtml(p.valor || '') + '</p>' +
      '</div>' +
      ...
      (p.status === 'pendente' ?
        ...
        '<button class="btn-aprovar" onclick="aprovarPagamento(\'' + p.id + '\', \'' + escaparHtml(p.email) + '\', \'' + escaparHtml(p.plano || p.plan_name || '') + '\')">Aprovar</button>' +
...
async function aprovarPagamento(id, email, plano) {
  if (!usuario.isAdmin || !_supabase) return;
  try {
    const dias = PLANOS_DURACAO[plano] || 30;
    const expira = new Date(Date.now() + dias * 24 * 60 * 60 * 1000).toISOString();

    await _supabase.from('payment_requests').update({ status: 'aprovado', approved_at: new Date().toISOString() }).eq('id', id);
    await _supabase.from('profiles').update({ is_vip: true, vip_plan: plano, vip_expires_at: expira }).eq('email', email);

    mostrarToast('Pagamento aprovado! VIP ativado para ' + email + ' (' + dias + ' dias).', 'sucesso');
    carregarSolicitacoesPagamento();
    carregarDashboardAdmin();
  } catch (e) {
...
async function recusarPagamento(id) {
  if (!usuario.isAdmin || !_supabase) return;
  try {
    await _supabase.from('payment_requests').update({ status: 'recusado' }).eq('id', id);
...
/* =========================================================================
   32. GERENCIAMENTO DE OBRAS — LISTAR, EDITAR, EXCLUIR, VIP, DESTAQUE
   ========================================================================= */
...
async function carregarGerenciamentoObras() {
  ...
  if (_supabase) {
    try {
      ...
      obras = (data || []).map(w => ({
        ...
        titulo: w.title,
        isVip: w.is_vip,
        destaque: w.is_featured,
        publicada: w.is_published
      }));
    } catch (e) {}
  }

  if (obras.length === 0) {
    container.innerHTML = '<p style="font-size:0.75rem; color:var(--texto-claro);">Nenhuma obra cadastrada.</p>';
    return;
  }
  ...
  container.innerHTML = obras.map(obra =>
    ...
    '<span style="font-size:0.75rem; font-weight:700; flex:1; min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">' + escaparHtml(obra.titulo) + '</span>' +
    ...
    '<button class="btn-aprovar" onclick="alternarPublicacaoObra(\'' + obra.id + '\',' + !obra.publicada + ')">' + (obra.publicada ? 'Ocultar' : 'Publicar') + '</button>' +
...
async function alternarVipObra(id) {
  ...
  try {
    const { data: obra } = await _supabase.from('works').select('is_vip').eq('id', id).single();
    if (!obra) return;
    const novoStatus = !obra.is_vip;
    await _supabase.from('works').update({ is_vip: novoStatus }).eq('id', id);
...
async function alternarPublicacaoObra(id, valor) {
  if (!usuario.isAdmin) return;
  try {
    await _supabase.from('works').update({ is_published: valor }).eq('id', id);
    mostrarToast(valor ? 'Obra publicada.' : 'Obra ocultada.', 'sucesso');
    carregarGerenciamentoObras();
    carregarObras();
  } catch (e) {
    mostrarToast('Erro: ' + e.message, 'erro');
...
function pedirExclusaoObra(id) {
  const obra = todasAsObras().find(o => String(o.id) === String(id));
...
async function adicionarObra(e) {
  e.preventDefault();
  if (!usuario.isAdmin) {
    mostrarToast('Sem permissão para cadastrar obras.', 'erro');
    return;
  }

  const tituloObra = document.getElementById('admTitulo').value.trim();
  if (!tituloObra) {
    mostrarToast('Digite o título da obra.', 'alerta');
    return;
  }
  ...
  if (!_supabase) {
    mostrarToast('Sem conexão com o banco de dados.', 'erro');
    return;
  }
  ...
  try {
    const { data: novaObra, error: errObra } = await _supabase.from('works').insert([{
      ...
      cover_url: capaUrl || CAPA_PLACEHOLDER,
      is_vip: isVipObra,
      is_published: true,
    ...
    await _supabase.from('chapters').insert([{
      ...
      pages: paginas.length > 0 ? paginas : [],
...
/* =========================================================================
   34. UPLOAD COM CONVERSÃO WEBP AUTOMÁTICA
   ========================================================================= */

function converterParaWebp(file, qualidade) {
...
async function uploadCapaSupabase(fileInput) {
  ...
  const file = await converterParaWebp(fileInput.files[0], 0.85);
...
async function uploadPaginasSupabase(fileList) {
  ...
  // Ordena pelo nome do arquivo (01, 02, 10, etc.)
  const arquivos = Array.from(fileList)
    .filter(f => f.type.startsWith('image/'))
    .sort((a, b) => a.name.localeCompare(b.name, undefined, { numeric: true }));

  for (const original of arquivos) {
    const file = await converterParaWebp(original, 0.8);
    ...
    if (!error) {
      ...
      mostrarToast('Falha em 1 página: ' + error.message, 'alerta');
    }
  }
  return urls;
}
...
async function uploadBannerSupabase(fileInput) {
  ...
  if (error) {
    mostrarToast('Erro no banner: ' + error.message, 'erro');
    return null;
  }
...
/* =========================================================================
   35. EDIÇÃO DE OBRA — MODAL COMPLETO
   ========================================================================= */
...
function garantirModalEditarObra() {
  ...
  modal.innerHTML = '<div class="modal-content" style="max-width:520px; width:94%; margin:auto; background:var(--fundo-card); border:1px solid var(--borda); border-radius:12px; padding:16px; max-height:85vh; overflow-y:auto;">' +
...
function garantirSelectsEquipe(modal) {
  ...
  div.innerHTML = '<div class="form-group"><label>Autor</label><select id="editObraAutor"></select></div>' +
...
/* =========================================================================
   36. GERENCIAMENTO DE CAPÍTULOS — AGENDAR, PUBLICAR, EXCLUIR
   ========================================================================= */
...
async function carregarGerenciamentoCapitulos() {
  ...
  sec.innerHTML = '<h4 style="font-size:0.9rem; color:var(--rosa-claro); margin-bottom:8px;"><i class="fa-solid fa-layer-group"></i> Gerenciar Capítulos</h4>' +
...
async function carregarCapitulosDaObra(workId) {
  ...
  container.innerHTML = data.map(c => {
    ...
    return '<div class="pay-item" style="margin-top:6px;">' +
...
/* =========================================================================
   37. GERENCIAMENTO DE CRIADORES E SCANLATORS
   ========================================================================= */
...
async function carregarGerenciamentoPessoas() {
  ...
  sec.innerHTML = '<h4 style="font-size:0.9rem; color:var(--rosa-claro); margin-bottom:8px;"><i class="fa-solid fa-users"></i> Criadores e Scanlators</h4>' +
...
async function carregarCreatorsAdmin() {
  ...
  if (!data || data.length === 0) {
    container.innerHTML = '<p style="font-size:0.7rem; color:var(--texto-claro);">Nenhum criador.</p>';
    return;
  }
  container.innerHTML = data.map(c =>
...
async function carregarScanlatorsAdmin() {
  ...
  if (!data || data.length === 0) {
    container.innerHTML = '<p style="font-size:0.7rem; color:var(--texto-claro);">Nenhuma scanlator.</p>';
    return;
  }
...
async function adicionarCreator() {
  ...
  if (!nome) {
    mostrarToast('Digite o nome do criador.', 'alerta');
    return;
  }
...
async function adicionarScanlator() {
  ...
  const nome = (input.value || '').trim();
  if (!nome) {
    mostrarToast('Digite o nome da scanlator.', 'alerta');
    return;
  }
...
/* =========================================================================
   38. BANNERS DO HERO — UPLOAD, LISTAR, EXCLUIR
   ========================================================================= */
...
async function carregarBannersAdmin() {
  ...
  sec.innerHTML = '<h4 style="font-size:0.9rem; color:var(--rosa-claro); margin-bottom:8px;"><i class="fa-solid fa-panorama"></i> Banners do Hero</h4>' +
...
async function adicionarBanner() {
  ...
  if (!fileInput || fileInput.files.length === 0) {
    mostrarToast('Escolha uma imagem para o banner.', 'alerta');
    return;
  }
  ...
  const { error } = await _supabase.storage.from('banners').upload(path, file, { contentType: file.type });
  if (error) {
    mostrarToast('Erro no upload: ' + error.message, 'erro');
    return;
  }
  ...
  const { error: errIns } = await _supabase.from('banners').insert([{
    title: titulo,
    image_url: url,
    is_active: true,
  ...
  if (errIns) {
    mostrarToast('Erro: ' + errIns.message, 'erro');
    return;
  }
...
/* =========================================================================
   39. MODERAÇÃO DE COMENTÁRIOS — OCULTAR, EXCLUIR
   ========================================================================= */
...
/* =========================================================================
   40. LISTAS PERSONALIZADAS — CARREGAMENTO E PADRÕES
   ========================================================================= */
...
/* =========================================================================
   41. ABAS DO PERFIL — HISTÓRICO, FAVORITOS, LISTAS, STATS
   ========================================================================= */
...
function renderizarAbasPerfil() {
  ...
  abasEl.innerHTML = '<button onclick="trocarAbaPerfil(\'historico\')" class="chip ' + (abaAtivaPerfil === 'historico' ? 'active' : '') + '" style="flex:1; min-width:70px;">Histórico</button>' +
  ...
  if (abaAtivaPerfil === 'historico') {
    ...
    const favsObras = todasAsObras().filter(o => usuario.favoritos.includes(o.id));
...
function renderizarPainelListas(painel) {
  painel.innerHTML = '<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">' +
    ...
    (listasPersonalizadas.length === 0
      ...
      : listasPersonalizadas.map(lista => {
          ...
          return '<div style="padding:10px; background:var(--fundo-card); border:1px solid var(--borda); border-radius:8px; margin-bottom:8px;">' +
...
function garantirModalListaDetalhada() {
  ...
  modal.innerHTML = '<div class="modal-content" style="max-width:420px; width:92%; margin:auto; background:var(--fundo-card); border:1px solid var(--borda); border-radius:12px; padding:16px; max-height:75vh; overflow-y:auto;">' +
...
async function salvarLista() {
  ...
  if (!nome) {
    mostrarToast('Digite o nome da lista.', 'alerta');
    return;
  }
  if (nome.length > 60) {
    mostrarToast('Nome muito longo (máximo 60 caracteres).', 'alerta');
    return;
  }
  ...
  if (_supabase && usuario.logado) {
    try {
      if (criando) {
        ...
        if (listaSalva) {
          listaSalva.nome = nome;
          listaSalva.descricao = desc;
        }
      }
    } catch (e) {
...
async function excluirLista(listaId) {
  ...
  if (!lista) return;
  if (lista.isPadrao) {
    mostrarToast('Listas padrão não podem ser excluídas.', 'alerta');
    return;
  }
...
function renderizarOpcoesEscolherLista(container, workId) {
  container.innerHTML = listasPersonalizadas.map(lista => {
    ...
    return '<button onclick="adicionarObraALista(\'' + lista.id + '\', \'' + workId + '\')" ' + (jaEsta ? 'disabled' : '') + ' style="width:100%; text-align:left; background:' + (jaEsta ? 'var(--fundo)' : 'var(--fundo-card)') + '; border:1px solid ' + (jaEsta ? 'var(--sucesso)' : 'var(--borda)') + '; border-radius:6px; padding:10px; margin-bottom:6px; cursor:' + (jaEsta ? 'default' : 'pointer') + '; display:flex; justify-content:space-between; align-items:center;">' +
      '<div style="flex:1; min-width:0;">' +
...
async function renderizarPainelStats(painel) {
  ...
  painel.innerHTML = '<div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(90px, 1fr)); gap:6px; margin-bottom:12px;">' +
    cards.map(c =>
      '<div style="background:var(--fundo-card); border:1px solid var(--borda); border-radius:8px; padding:8px; text-align:center;">' +
...
/* =========================================================================
   45. EXCLUSÃO DE CONTA (ZONA DE PERIGO)
   ========================================================================= */
...
function garantirZonaPerigoPerfil() {
  ...
  sec.innerHTML = '<h4 style="font-size:0.8rem; color:var(--erro); margin:0 0 6px;">Zona de Perigo</h4>' +
...
function garantirModalExclusaoConta() {
  ...
  modal.innerHTML = '<div class="modal-content" style="max-width:420px; width:92%; margin:auto; background:var(--fundo-card); border:1px solid var(--erro); border-radius:12px; padding:16px;">' +
...
/* =========================================================================
   46. NOTIFICAÇÕES DE CAPÍTULOS NOVOS
   ========================================================================= */
...
async function carregarNotificacoes() {
  ...
  if (notifs.length === 0) {
    todasAsObras().forEach(o => (o.capitulos || []).forEach(c => {
      notifs.push({
        id: String(c.id),
        obraId: o.id,
        titulo: o.titulo,
...
function abrirNotificacoes() {
  ...
  if (listaNotificacoes.length === 0) {
    ...
    container.innerHTML = listaNotificacoes.map(n =>
      '<div class="notif-item" onclick="abrirObraDaNotificacao(\'' + n.obraId + '\')" style="padding:10px; background:var(--fundo-card); border:1px solid var(--borda); border-radius:8px; margin-bottom:6px; display:flex; gap:10px; cursor:pointer;">' +
      '<div class="notif-icon" style="color:var(--primaria);"><i class="fa-solid fa-book-open"></i></div>' +
      '<div>' +
      '<p style="font-size:0.8rem; font-weight:700; margin:0;">' + escaparHtml(n.titulo) + '</p>' +
      '<p style="font-size:0.7rem; color:var(--texto-claro); margin:2px 0 0;">' + escaparHtml(n.texto) + '</p>' +
      '</div></div>'
    ).join('');
  }
...
/* =========================================================================
   47. INFORMAÇÕES LEGAIS (TERMOS, PRIVACIDADE, DMCA)
   ========================================================================= */
...
/* =========================================================================
   48. PWA — SERVICE WORKER E INSTALAÇÃO
   ========================================================================= */
...
/* =========================================================================
   49. ACESSIBILIDADE — SKIP LINK, ARIA, REDUCED MOTION
   ========================================================================= */
...
function aplicarAcessibilidade() {
  ...
  if (!window.__enterCardAtivo) {
    ...
    document.addEventListener('keydown', e => {
      if (e.key === 'Enter' && document.activeElement && document.activeElement.classList && document.activeElement.classList.contains('manga-card')) {
        e.preventDefault();
...
/* =========================================================================
   52. INICIALIZAÇÃO DA PÁGINA (DOMContentLoaded)
   ========================================================================= */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Tema automático por horário (18h-6h)
  ...
  // Aplicar overrides locais (obras removidas, VIP local)
...
/* =========================================================================
   FIM DO ARQUIVO — SOLITUDESCAN v3.0
   Todas as 6 partes consolidadas. Sem patches sobrepostos.
   ========================================================================= */

/* =========================================================================
   SOLITUDESCAN — SERVICE WORKER v4
   Estratégia:
   - Navegação: network-first (sempre tenta o novo index.html)
   - Estáticos mesmos domínio: stale-while-revalidate
   - CDNs (fontes/ícones): stale-while-revalidate
   - Supabase (API/auth/storage): NUNCA intercepta (network puro)
   ========================================================================= */

const CACHE_VERSION = 'solitude-v4';
const CACHE_CORE = CACHE_VERSION + '-core';
const CACHE_RUNTIME = CACHE_VERSION + '-runtime';
const MAX_RUNTIME_ITENS = 60;
  ...
  event.waitUntil(
    caches.keys()
      .then((keys) => Promise.all(
        keys
          .filter((key) => key !== CACHE_CORE && key !== CACHE_RUNTIME)
...
async function limitarCacheRuntime(cache) {
  const keys = await cache.keys();
  if (keys.length > MAX_RUNTIME_ITENS) {
...
self.addEventListener('fetch', (event) => {
  ...
  // Supabase (API, auth, storage): nunca interceptar
    ...
    event.respondWith(
      fetch(request)
        .then((response) => {
          ...
          caches.open(CACHE_CORE).then((cache) => cache.put('/index.html', copia));
          return response;
        })
  ...
  // Estáticos do próprio domínio + CDNs confiáveis: stale-while-revalidate
  const ehMesmoDominio = url.origin === self.location.origin;
  const ehCdnConfiavel = (
    url.hostname.includes('cdnjs.cloudflare.com') ||
    url.hostname.includes('cdn.jsdelivr.net') ||
    url.hostname.includes('fonts.googleapis.com') ||
    url.hostname.includes('fonts.gstatic.com')
  );
  const ehAsset = /\.(css|js|svg|png|webp|woff2?)$/i.test(url.pathname);

  if (ehMesmoDominio || (ehCdnConfiavel && ehAsset)) {
    event.respondWith(
